...
test.describe('読書管理アプリ - ホームページ', () => {
  ...
  test('年フィルターが表示され、初期状態でAllが選択されている', async ({ page }) => {
    // 年フィルターが存在することを確認
    await expect(page.getByRole('button', { name: 'すべての年の本を表示' })).toBeVisible()
    await expect(page.getByRole('button', { name: '2024年の本を表示' })).toBeVisible()
  ...
  test('年フィルターが正しく動作する', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '2024年の本を表示' })).toHaveAttribute('aria-selected', 'true')
  ...
  test('キーボードナビゲーションが正しく動作する', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'すべての年の本を表示' })).toBeFocused()
  ...
  test('無限スクロールが動作する（本が多数ある場合）', async ({ page }) => {
    // 初期表示の本の数を取得
    const initialBookCount = await page.locator('.card').count()
