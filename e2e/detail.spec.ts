...
test.describe('読書管理アプリ - 詳細ページ', () => {
  ...
  test('存在しない本IDの場合、エラーメッセージが表示される', async ({ page }) => {
    // 存在しない本IDのページに直接アクセス
  ...
  test('複数の本で詳細ページが正しく動作する', async ({ page }) => {
    ...
    // 最初の本をクリック
    const firstBookTitle = await page.locator('.card').first().locator('h2').textContent()
    await page.locator('.card').first().click()
    await expect(page).toHaveURL(/\/items\//)
    ...
    if (cardCount > 1) {
      const secondBookTitle = await page.locator('.card').nth(1).locator('h2').textContent()
