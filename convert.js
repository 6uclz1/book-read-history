...
try {
    ...
    // 先頭にヘッダー行を追加
    const booksHeader = 'title,_0,author,_,publisher,isbn,readDate,_1,_2,thumnailImage,_3,_4,_5,_6\n'
    ...
    const highlightsD = parse(highlightsData, {columns: ['Book','ASIN','Section','Location','Highlight','Note'], record_delimiter: ['\r\n', '\r']})
    ...
    const formattedData = booksD.map((row, index) => {
        ...
        if (highlights.length === 0 && row.title) {
            try {
                const bookTitleRegex = new RegExp(escapeRegExp(normalizeTitle(row.title)));
                let matchedHighlights = [];
                for (const hTitle in highlightsByTitle) {
                    if (bookTitleRegex.test(hTitle)) {
                        matchedHighlights = matchedHighlights.concat(highlightsByTitle[hTitle]);
                    }
                }
        ...
        if(highlights.length > 0) {
            console.log(`[OK] ${row.title} (ISBN13: ${row.isbn}, ASIN: ${asin}) - ${highlights.length}件のハイライトが見つかりました`);
        } else {
