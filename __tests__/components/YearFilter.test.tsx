...
describe('YearFilter', () => {
  ...
  it('選択された年のボタンが選択状態になっている', () => {
    ...
    const selectedButton = screen.getByRole('tab', { name: '2024年の本を表示' })
    expect(selectedButton).toHaveAttribute('aria-selected', 'true')

    const unselectedButton = screen.getByRole('tab', { name: '2023年の本を表示' })
  ...
  it('選択された年がAllの場合、適切にaria-selectedが設定される', () => {
    ...
    const otherButtons = screen.getAllByRole('tab').filter(tab => tab !== allButton)
  ...
  it('動的に生成された年リストに対応している', () => {
    ...
    expect(screen.getByRole('tab', { name: 'すべての年の本を表示' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: '2025年の本を表示' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: '2024年の本を表示' })).toBeInTheDocument()
    expect(screen.queryByRole('tab', { name: '2023年の本を表示' })).not.toBeInTheDocument()
  })
})